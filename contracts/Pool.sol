...
contract Pool {
    ...
    event LendLog(address indexed sender, uint256 amount, uint256 timestamp, string category, Person person);
    event BorrowLog(address indexed borrower, uint256 amount, uint256 timestamp, string category, Person person);
    ...
    function lend(uint256 timestamp, string memory category, uint256 age, string memory gender, string memory location) public payable {
        lenders[msg.sender].push(Entry(msg.value, timestamp, category, Person(age, gender, location), true));
        emit LendLog(msg.sender, msg.value, timestamp, category, Person(age, gender, location));
    }
    ...
    function borrow(uint256 amount, uint256 timestamp, string memory category, uint256 age, string memory gender, string memory location) public payable {
        require(amount <= address(this).balance);
        borrowers[msg.sender].push(Entry(amount, timestamp, category, Person(age, gender, location), true));
        payable(msg.sender).transfer(amount);
        emit BorrowLog(msg.sender, amount, timestamp, category, Person(age, gender, location));
    }
